...
export default function AIChatDeleteDialog({ chat, isOpen, onClose, onConfirm }) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl border border-slate-200 p-5 text-center animate-in zoom-in-95 duration-150">
        <div className="w-11 h-11 rounded-xl bg-red-50 text-red-600 flex items-center justify-center mx-auto mb-3">
          <FiTrash2 className="w-5 h-5" />
        </div>

        <h4 id="delete-chat-title" className="text-sm font-bold text-slate-900 mb-1">
        ...
        <div className="flex items-center justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 px-3 py-2 text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors cursor-pointer"
          >
            Cancel
          </button>
          <button
